...
public class CompraDAO {
    ...
    String sql = "INSERT INTO compras_oro (cliente_id, usuario_id, peso_gramos, kilate, punto, precio_gramo, total,imagen, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    try (Connection con = ConexionDB.obtenerConexion();
        ...
        if (compra.getImagen() != null) {
            ps.setBytes(8, compra.getImagen());
        } else {
            ps.setNull(8, java.sql.Types.LONGVARBINARY);
        }
        ps.setString(9, compra.getEstado());
        ps.executeUpdate();
    } catch (Exception e) {
        e.printStackTrace();
    ...
    public List<Compra> obtenerPorUsuario(int usuarioId) {
        List<Compra> lista = new ArrayList<>();
        String sql = "SELECT co.id, c.nombre AS nombre_cliente, co.peso_gramos, co.kilate, co.punto, co.precio_gramo, co.total, co.fecha, co.imagen, co.estado " +
                     "FROM compras_oro co " +
                     "JOIN clientes c ON co.cliente_id = c.id " +
                     "WHERE co.usuario_id = ? " +
                     "ORDER BY co.fecha DESC";
        try (Connection con = ConexionDB.obtenerConexion();
            ...
            while (rs.next()) {
                ...
                comp.setImagen(rs.getBytes("imagen")); // <<< NUEVO
                comp.setEstado(rs.getString("estado"));
                lista.add(comp);
            }
        } catch (Exception e) {
    ...
    public Compra obtenerPorId(int id) {
        String sql = "SELECT co.id, co.cliente_id, co.usuario_id, c.nombre AS nombre_cliente, co.peso_gramos, co.kilate, co.punto, " +
                     "co.precio_gramo, co.total, co.imagen, co.fecha, co.estado " +
                     "FROM compras_oro co " +
                     "JOIN clientes c ON co.cliente_id = c.id " +
                     "WHERE co.id = ?";
        try (Connection con = ConexionDB.obtenerConexion();
            ...
            if (rs.next()) {
                ...
                c.setImagen(rs.getBytes("imagen")); // <<< NUEVO
