...
public class CompraDAO {
    ...
    String sql = "INSERT INTO compras_oro (cliente_id, usuario_id, peso_gramos, kilate, punto, precio_gramo, total, observaciones) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
...
public List<Compra> obtenerPorUsuario(int usuarioId) {
    List<Compra> lista = new ArrayList<>();
    String sql = "SELECT c.nombre AS nombre_cliente, co.peso_gramos, co.kilate, co.punto, co.precio_gramo, co.total, co.fecha " +
                 "FROM compras_oro co " +
                 "JOIN clientes c ON co.cliente_id = c.id " +
                 "WHERE co.usuario_id = ? " +
                 "ORDER BY co.fecha DESC";
